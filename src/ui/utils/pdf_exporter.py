...
class PdfExporter:
    ...
    def export_to_pdf(self, worker_data, filepath, output_folder):
        ...
        for index, (name, worker) in enumerate(worker_data.items()):
            ...
            # 名前の描画位置を調整
            self.draw_worker_name(c, name, center_x, line_y) # 修正
            if (index + 1) % 8 == 0:
               c.showPage()
               # 新しいページも白背景に設定
               c.setFillColor(colors.white)
    ...
    def draw_radar_chart(self, c, center_x, center_y, radius, categories, data, name):
        ...
        angles.append(angles[0])  # 最初の角度を追加 (ポリゴンを閉じるため)
        data_with_closure = data + data[:1] # 新しいリストとして作成
        logger.debug(f"--- PdfExporter: draw_radar_chart: angles: {angles}")
        logger.debug(f"--- PdfExporter: draw_radar_chart: data: {data_with_closure}")

        # スケールを描画 (円と軸)
        self.draw_chart_scale(c, center_x, center_y, radius, len(categories))

        # データポイントを計算
        points = self.calculate_data_points(data_with_closure, angles, center_x, center_y, radius)
        logger.debug(f"--- PdfExporter: draw_radar_chart: calculate_data_points points: {points}")

        # 塗りつぶしと輪郭の描画
        self.draw_chart_polygon(c, points, colors.Color(0, 0, 1, alpha=0.15))

        # カテゴリラベルを追加
        self.draw_category_labels(c, categories, angles, center_x, center_y, radius)

    def draw_chart_scale(self, c, center_x, center_y, radius, num_axes):
        """チャートのスケール（円と軸）を描画"""
        c.setLineWidth(0.3)
    ...
    def calculate_data_points(self, data, angles, center_x, center_y, radius):
        ...
        for i, d in enumerate(data):
            scaled_radius = radius * (d / 5)
            angle = angles[i]
            x = center_x + scaled_radius * np.cos(angle)
            y = center_y + scaled_radius * np.sin(angle)
            points.append((float(x), float(y)))# 修正
        logger.debug(f"--- PdfExporter: calculate_data_points: points: {points}")
        return points

    def draw_chart_polygon(self, c, points, fill_color):
        """チャートのポリゴンを描画"""
        c.setFillColor(fill_color)
        c.setStrokeColor(colors.blue)
        c.setLineWidth(1.5)
        polygon = Polygon(points)
        c.drawPath(polygon, fill=1, stroke=1)
